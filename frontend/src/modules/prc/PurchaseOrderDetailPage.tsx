...
export default function PurchaseOrderDetailPage() {
  ...
  async function handleReceive(e: React.FormEvent) {
    ...
    const res = await createGRN.mutateAsync({ lines });
                  ...
                  {po.latest_match.match_status === "discrepancy" && (
                    <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const matchId = po.latest_match?.id;
                          if (matchId) approveException.mutate({ matchId, reason: exceptionReason });
                        }}
                      >
                        Approve exception
                      </Button>
