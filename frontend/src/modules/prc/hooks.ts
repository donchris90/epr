...
export function useVendors() {
  return useQuery({
    ...
    queryFn: async () => (await apiClient.get<{ data: Vendor[] }>("/prc/vendors")).data.data,
  });
}
...
export function useCreateVendor() {
  ...
  return useMutation({
    mutationFn: (payload: { name: string; tax_registration_number?: string }) =>
      apiClient.post<Vendor>("/prc/vendors", payload),
...
export function usePurchaseRequests(status?: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await apiClient.get<{ data: PurchaseRequest[] }>("/prc/purchase-requests", { params: status ? { status } : {} }))
        .data.data,
  });
}
...
export function useCreatePurchaseRequest() {
  ...
  return useMutation({
    ...
    }) => apiClient.post<PurchaseRequest>("/prc/purchase-requests", payload),
...
export function useSubmitPurchaseRequest(prId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: { remaining_budget?: string; override?: boolean; override_reason?: string }) =>
      apiClient.post<PurchaseRequest>(`/prc/purchase-requests/${prId}/submit`, payload),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-requests"] }),
  });
}
...
export function useApprovePurchaseRequest() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (prId: string) => apiClient.post<PurchaseRequest>(`/prc/purchase-requests/${prId}/approve`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-requests"] }),
  });
}
...
export function usePurchaseOrders(status?: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await apiClient.get<{ data: PurchaseOrder[] }>("/prc/purchase-orders", { params: status ? { status } : {} }))
        .data.data,
  });
}
...
export function usePurchaseOrder(poId?: string) {
  return useQuery({
    ...
    queryFn: async () => (await apiClient.get<PurchaseOrderDetail>(`/prc/purchase-orders/${poId}`)).data,
    enabled: !!poId,
  });
}
...
export function useCreatePurchaseOrder() {
  ...
  return useMutation({
    ...
    }) => apiClient.post<PurchaseOrder>("/prc/purchase-orders", payload),
...
export function useInitiatePOApproval(poId?: string) {
  ...
  return useMutation({
    mutationFn: (thresholds: { value_threshold: string | null; role_required: string }[]) =>
      apiClient.post<PurchaseOrder>(`/prc/purchase-orders/${poId}/approval-workflow/initiate`, { thresholds }),
...
export function useDecidePOApprovalStep(poId?: string) {
  ...
  return useMutation({
    mutationFn: ({ stepId, decision, comments }: { stepId: string; decision: "approved" | "rejected"; comments?: string }) =>
      apiClient.post<POApprovalStep>(`/prc/po-approval-steps/${stepId}/decide`, { decision, comments }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
...
export function useIssuePurchaseOrder(poId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: { waiver?: boolean; waiver_reason?: string }) =>
      apiClient.post<PurchaseOrder>(`/prc/purchase-orders/${poId}/issue`, payload),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
...
export function useCreateGRN(poId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: { lines: { po_line_id: string; quantity_received: string; condition?: string; discrepancy_notes?: string }[] }) =>
      apiClient.post<GoodsReceiptNote>("/prc/goods-receipt-notes", { purchase_order_id: poId, ...payload }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
...
export function useConfirmGRN(poId?: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (grnId: string) => apiClient.post<GoodsReceiptNote>(`/prc/goods-receipt-notes/${grnId}/confirm`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
...
export function useCreateInvoiceMatch(poId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: { goods_receipt_note_id?: string; vendor_invoice_reference: string; invoice_amount: string }) =>
      apiClient.post<InvoiceMatch>(`/prc/purchase-orders/${poId}/invoice-match`, payload),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
...
export function useApproveMatchException(poId?: string) {
  ...
  return useMutation({
    mutationFn: ({ matchId, reason }: { matchId: string; reason: string }) =>
      apiClient.post<InvoiceMatch>(`/prc/invoice-matches/${matchId}/approve-exception`, { reason }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["prc", "purchase-orders", "detail", poId] }),
  });
}
