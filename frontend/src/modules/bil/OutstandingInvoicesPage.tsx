...
export default function OutstandingInvoicesPage() {
  const query = useOutstandingInvoices();
  ...
  return (
    <div>
      ...
      <QueryState
        query={query}
        variant="dashboard"
        loadingLabel="Loading outstanding invoices"
        emptyTitle="Nothing outstanding"
        emptyHint="Every submitted certificate has either been paid or hasn't reached its due date."
        isEmpty={(bands) => !Object.values(bands ?? {}).some((items: any) => items?.length > 0)}
      >
        {(data) => {
          const bands: Record<string, any[]> = data ?? {};
          return (
            <>
              {Object.entries(BAND_LABELS).map(([band, label]) => {
                ...
                return (
                  <Card key={band} style={{ marginBottom: 16, padding: 0 }}>
                    ...
                    <Table ariaLabel={`${label} invoices`}>
                      <thead>
                        <tr>
                          <Th>Certificate</Th>
                          <Th>Amount</Th>
