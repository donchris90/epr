...
export function useClients() {
  return useQuery({
    ...
    queryFn: async () => (await apiClient.get("/bdc/clients")).data.data,
  });
}
...
export function useCreateClient() {
  ...
  return useMutation({
    mutationFn: (payload: { name: string; billing_email?: string }) =>
      apiClient.post("/bdc/clients", payload),
...
export function useLeads() {
  return useQuery({
    queryKey: ["bdc", "leads"],
    queryFn: async () => (await apiClient.get("/bdc/leads")).data.data,
  });
}
...
export function useCreateLead() {
  ...
  return useMutation({
    mutationFn: (payload: { name: string; source?: string; estimated_value?: string }) =>
      apiClient.post("/bdc/leads", payload),
...
export function useConvertLead() {
  ...
  return useMutation({
    mutationFn: ({ leadId, clientId }: { leadId: string; clientId: string }) =>
      apiClient.post(`/bdc/leads/${leadId}/convert`, { client_id: clientId }),
    onSuccess: () => {
...
export function useOpportunities() {
  return useQuery({
    ...
    queryFn: async () => (await apiClient.get("/bdc/opportunities")).data.data,
  });
}
...
export function useTransitionOpportunity() {
  ...
  return useMutation({
    mutationFn: ({ id, newStage }: { id: string; newStage: string }) =>
      apiClient.post(`/bdc/opportunities/${id}/transition`, { new_stage: newStage }),
...
export function useBidNoBidDecision() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiClient.post(`/bdc/opportunities/${id}/bid-no-bid`, {
        decision,
        scorecard: {},
        rationale,
