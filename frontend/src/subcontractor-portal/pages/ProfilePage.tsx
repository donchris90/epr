...
import { useSubcontractorProfile, changeSubcontractorPassword, logoutSubcontractor } from "../hooks";
import { getSubcontractorPortalErrorMessage } from "../api/client";

export default function ProfilePage() {
  const navigate = useNavigate();
  const { profile, error: profileError } = useSubcontractorProfile();
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await changeSubcontractorPassword(currentPassword, newPassword);
      setSuccess(true);
      setTimeout(async () => {
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      <PageHeader eyebrow="Subcontractor Portal" title="My Account" />
