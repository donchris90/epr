...
@bp.post("/contracts/<uuid:contract_id>/amendments/<uuid:amendment_id>/finalize")
@require_permission("ctm:approve")
def finalize_amendment(contract_id, amendment_id):
    """Applies a pending amendment's deferred contract mutation once
    its governing workflow instance reports approved -- see
    services.py:finalize_amendment for what this defers to while
    still pending, and app/workflow/ for the engine itself."""
    _get_contract_or_404(contract_id)
    amendment = ContractAmendment.query.filter_by(id=amendment_id, tenant_id=g.tenant_id, contract_id=contract_id).first()
    if not amendment:
        raise APIError("Amendment not found", status=404)
    amendment = services.finalize_amendment(amendment, actor_id=g.user_id)
    return jsonify(amendment_schema.dump(amendment))
