...
def win_loss_summary(tenant_id, *, group_by="client"):
    """
    BDC-11: Win/Loss reports summarizing conversion rate by client or
    value band. "sector" is not implemented and raises explicitly
    rather than silently returning an empty or wrong result --
    Client (BDC-02) has no sector/industry field anywhere in this
    schema, so there is genuinely nothing to group by. Adding real
    sector grouping means adding that field to Client first, which is
    a real, separate, honest gap, not something to fake here.
    """
    from sqlalchemy import func, case
    from app.modules.bdc.models import Client, Opportunity, WinLossRecord

    if group_by not in ("client", "value_band"):
        if group_by == "sector":
            raise APIError(
                "Grouping by sector is not implemented",
                status=400,
                detail="Client (BDC-02) has no sector/industry field in this schema yet -- there is nothing to group by.",
            )
        raise APIError(f"Invalid group_by value: {group_by!r}", status=400, detail="Must be one of: client, value_band")

    won_count = func.count(case((WinLossRecord.outcome == "won", 1)))
    lost_count = func.count(case((WinLossRecord.outcome == "lost", 1)))
    total_count = func.count(WinLossRecord.id)
    won_value = func.coalesce(
        func.sum(case((WinLossRecord.outcome == "won", Opportunity.estimated_value), else_=0)), 0
    )
    ...
    if group_by == "client":
        rows = (
            db.session.query(
                ...
                won_count.label("won"),
                lost_count.label("lost"),
                total_count.label("total"),
                won_value.label("won_value"),
            )
            .join(Opportunity, Opportunity.client_id == Client.id)
            .join(WinLossRecord, WinLossRecord.opportunity_id == Opportunity.id)
            .filter(WinLossRecord.tenant_id == tenant_id, Client.tenant_id == tenant_id)
            .group_by(Client.id, Client.name)
            .order_by(Client.name)
            .all()
        )
    else:  # value_band -- fixed, documented bands, not user-configurable in this pass
        band_expr = case(
            (Opportunity.estimated_value < 10_000_000, "< 10M"),
            (Opportunity.estimated_value < 50_000_000, "10M - 50M"),
            (Opportunity.estimated_value < 200_000_000, "50M - 200M"),
            else_="200M+",
        )
        rows = (
            db.session.query(
                band_expr.label("group_key"),
                band_expr.label("group_label"),
                won_count.label("won"),
                lost_count.label("lost"),
                total_count.label("total"),
                won_value.label("won_value"),
            )
            .join(Opportunity, Opportunity.id == WinLossRecord.opportunity_id)
            .filter(WinLossRecord.tenant_id == tenant_id)
            .group_by(band_expr)
            .all()
        )
    ...
    return [
        {
            ...
            "lost": r.lost,
            "total": r.total,
            "win_rate": round(r.won / r.total, 4) if r.total else 0,
