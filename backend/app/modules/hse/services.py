...
def issue_permit_to_work(
    ...
    *,
    ...
    Real safety-control gap found and fixed while wiring this up, not
    invented to justify the work: every permit -- including hot work,
    confined space entry, and working at height -- was created with
    status="approved" immediately, with approved_by never even set
    (the route didn't pass an actor at all). Any single user holding
    hse:write could self-issue an "approved" permit for hazardous work
    with zero recorded accountability for who approved it. Two
    independent fixes, both effective regardless of whether a
    workflow is configured: (1) actor_id is now required and recorded
    as approved_by even in the no-workflow path -- pure accountability,
    no gating change; (2) when a tenant configures and activates a
    Workflow Engine chain for ("hse", "permit_to_work"), the permit is
    created "draft" instead and a real approval instance governs it --
    see finalize_permit_approval for the deferred half.
    """
    ...
    workflow = workflow_services.get_active_workflow(tenant_id, module_name="hse", entity_type="permit_to_work")
    ...
    if workflow:
        ...
        workflow_services.start_workflow_instance(
            tenant_id, workflow,
            module_name="hse", entity_type="permit_to_work", entity_id=permit.id,
...
def finalize_permit_approval(permit: PermitToWork, *, actor_id):
    """
    Marks a draft permit approved once the workflow governing it
    reports approved -- the deferred half of issue_permit_to_work,
    mirroring the pattern used for PRC Purchase Requests and CTM
    Contract Amendments.
    """
    if permit.status != "draft":
        raise APIError(f"Permit is not in draft (current status: {permit.status})", status=409)

    from app.workflow.models import WorkflowInstance

    instance = (
        WorkflowInstance.query.filter_by(
            tenant_id=permit.tenant_id, module_name="hse", entity_type="permit_to_work", entity_id=permit.id
        )
    ...
    if instance and instance.status == "pending":
        raise APIError(
            "This permit is governed by an approval workflow",
            status=409,
            detail=(
    ...
    if instance and instance.status in ("rejected", "cancelled"):
        raise APIError(f"The governing approval workflow was {instance.status} for this permit", status=409)

    permit.status = "approved"
    permit.approved_by = actor_id
