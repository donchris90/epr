...
def apply_transfer_cutover(transfer: EquipmentTransfer):
    """The actual effect of a cutover -- factored out so both an
    immediate (same-day) cutover at approval time and a later one
    applied by the Celery task (app/modules/eqp/tasks.py) for a
    future-dated transfer go through the exact same code path."""
    transfer.equipment.current_project_id = transfer.to_project_id
    transfer.cutover_applied_at = datetime.now(timezone.utc)
...
def approve_transfer(transfer: EquipmentTransfer, *, approved_by, cutover_date=None):
    ...
    # effect immediately; a future-dated cutover is applied later by
    # the daily Celery task once that date arrives (previously a real
    # gap: no task existed for this at all -- see
    # app/modules/eqp/tasks.py).
    if transfer.cutover_date <= date.today():
        apply_transfer_cutover(transfer)

    db.session.commit()
    return transfer


def due_unapplied_transfer_cutovers(tenant_id, *, as_of=None):
    """Approved transfers whose cutover_date has arrived (or passed)
    but whose real effect hasn't been applied yet -- what
    approve_transfer intentionally leaves for the Celery task
    (app/modules/eqp/tasks.py) when the cutover was future-dated at
    approval time."""
    as_of = as_of or date.today()
    return EquipmentTransfer.query.filter(
        EquipmentTransfer.tenant_id == tenant_id,
        EquipmentTransfer.status == "approved",
        EquipmentTransfer.cutover_applied_at.is_(None),
        EquipmentTransfer.cutover_date <= as_of,
    ).all()
