...
class EquipmentTransfer(db.Model, UUIDPrimaryKeyMixin, TenantMixin, AuditMixin):
    ...
    # Set the moment the cost-allocation cutover (equipment.current_project_id
    # switching to to_project_id) actually happens -- immediately at
    # approval for a same-day cutover, or later by the daily Celery
    # task (app/modules/eqp/tasks.py) for a future-dated one. Distinct
    # from `status` staying "approved": this tracks whether the real
    # effect has happened yet, not whether the transfer was approved.
    cutover_applied_at = db.Column(db.DateTime(timezone=True), nullable=True)
