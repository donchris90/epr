...
class BaseConfig:
    ...
    # Email dispatch (app/notifications/email.py, app/notifications/tasks.py)
    # -- Gmail SMTP specifically needs an App Password
    # (https://myaccount.google.com/apppasswords), not the regular
    # account password: Google requires 2-Step Verification to be on
    # before an App Password can even be generated, and rejects plain
    # account-password SMTP auth from third-party apps outright.
    # Empty by default: notifications/email.py's send_email() no-ops
    # with a clear log line (not a crash) when SMTP_USERNAME is unset,
    # so local dev and any tenant that hasn't configured this yet
    # keep working -- in-app notifications alone don't need SMTP.
