...
- **No notifications of any kind** -- email, in-app, or push. This
  codebase has no notification system at all to integrate with yet.
  Every action is recorded and queryable via the API
  (`GET /v1/workflow/instances/pending`, etc.), but nothing pushes
  anything to anyone.
...
The pattern PRC uses, generalized:
...
2. At the point where an entity is submitted for approval, check
   ...
   routing.
3. At the point where the module's own "approve" logic would run,
   check whether a `WorkflowInstance` exists for this entity. If its
   status is `pending`, defer (return an error pointing at the real
   workflow endpoint, as PRC does). If `approved`, proceed with the
   entity's own finalization. If `rejected`/`cancelled`, reflect that.
