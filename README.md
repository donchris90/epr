...
Found via an actual gap audit, not assumed: checking every listed
phase item against the real, deployed codebase turned up one genuine
missing piece among otherwise near-total coverage -- Subcontractors
(Module 12/SUB) had full internal management (agreements, progress,
payment certificates, retention, claims) but, unlike Clients (CLP) and
Vendors (VNP), no actual portal for subcontractors to interact
themselves.

Built following the exact CLP/VNP pattern -- specifically closer to
VNP's shape, since a subcontractor portal user belongs to exactly one
subcontractor (the simple one-to-one ownership check), not CLP's
many-to-many client-to-project scoping. Only one new table
(`SubcontractorPortalUser`) was needed: Module 12's own
`SubcontractProgressEntry` and `SubcontractClaim` were already
designed anticipating subcontractor-submitted data (the former's
`submitted_by` field is literally documented as "the subcontractor's
representative"), so the portal writes directly to those existing
tables through ownership-checked service functions rather than
duplicating them -- the same reuse-over-redesign principle CLP and VNP
already established.

Every route enforces subcontractor-ownership as an unconditional first
check, independent of the caller's permission grant -- the same
defense-in-depth pattern as CLP's client-scope check and VNP's
vendor-ownership check. Verified for real against Postgres, including
the ownership boundary specifically: seeded two real subcontractors,
confirmed Subcontractor A's portal user can submit progress, view
their own payment certificates, and submit a claim, and confirmed
Subcontractor B's portal user is correctly blocked (403) from every
one of those same actions against Subcontractor A's agreement. 9
tests, plus the full 76-test tenant-isolation suite re-run afterward
confirming nothing else regressed. Migration re-verified using only
the ordinary non-privileged database role, the same production-safety
check every migration has gotten since the login-outage lesson earlier
in this build.

## Notifications, and a real integration into the Workflow Engine

Closes a gap flagged repeatedly since the Workflow Engine session:
this platform had zero notification infrastructure of any kind --
every module's audit trail recorded that something happened, but
nothing ever told anyone.

Adapted from a real, working implementation found in the user-provided
reference codebase (`epr-phase2-commercial-core.zip`, generated
independently against the same underlying SRS) -- following the
recommendation to mine it for genuinely good pieces rather than
discard it. Correcting it to match this codebase's own conventions
turned up a real inconsistency, not just cosmetic differences: the
source had a hard foreign key from `Notification.user_id` to
`users.id`, but every other "who does this belong to" field across
this entire platform (`WorkflowAction.actor_id`,
`WorkflowInstance.initiated_by`, and the pattern used everywhere else)
is a loose reference with no FK. Fixed to match -- confirmed
necessary, not just theoretical, when the first test run hit a real
foreign-key violation from a JWT-claim-only user with no backing
`User` row, exactly the scenario a hard FK would make brittle in
practice.

Real integration into the Workflow Engine, not standalone
infrastructure sitting unused: starting an instance notifies every
real user who can act on its first step (a named user directly, or
everyone actually holding the required Role, resolved by a genuine
query against the `users` table -- not just whoever happens to be
logged in); advancing to a new step notifies that step's approvers;
full approval notifies the initiator; rejection notifies the
initiator, or -- on a reject-to-step configuration -- the earlier
step's approvers instead. Verified end-to-end against real Postgres:
seeded two users sharing one role, confirmed both received a real
notification when an instance started, confirmed the initiator was
notified on approval, confirmed marking one read correctly dropped the
unread count.

8 new tests, plus the existing 12-test workflow suite and the full
76-test tenant-isolation suite both re-run afterward confirming
nothing regressed. Migration re-verified using only the ordinary
non-privileged database role, the same production-safety check every
migration has gotten since the login-outage lesson earlier in this
build.

Update: email dispatch (`_dispatch_external`) is no longer a no-op --
see the "Real email dispatch via Gmail SMTP" session note further down
for the real implementation added in a later session. SMS remains
unimplemented (the user explicitly chose email over SMS as the
notification channel).

## Commitment accounting -- and closing a gap PRC's own code had been documenting since it was written

The last of the three Phase 6 items checked against this codebase and
found genuinely missing. Closing it surfaced something worth stating
plainly: PRC's `submit_purchase_request` docstring had said, since
that function was first written, that `remaining_budget` is "supplied
by the caller (not looked up here)" -- meaning no real computation of
remaining budget existed anywhere in this platform. Every budget check
this whole build has ever done depended on some future caller
supplying a number nothing actually calculated.

`app/commitments/` closes it -- deliberately with no new table.
Committed spend is computed live from existing source-of-truth data
(`PurchaseOrder` + `PurchaseOrderLine`, filtered to
approved/issued/closed status -- draft and cancelled POs were never
real commitments), not a denormalized ledger that would need
synchronizing and could drift stale. Scope stated honestly, not
oversold: this computes *committed* spend against budget, not also
netted against *actual* invoiced spend from Accounts Payable -- AP
invoices in this schema reference a whole Purchase Order, not a
specific line or CBS code, so apportioning one back to a single CBS
line when a PO spans several would need an allocation rule this
codebase doesn't have. What it answers completely and correctly: how
much of a budget line is already spoken for by issued POs, which is
what "Commitments" means in cost control.

Real integration, not a parallel unused endpoint: PRC's
`submit_purchase_request` now computes a real remaining-budget figure
via this module when the caller doesn't supply one explicitly, instead
of silently skipping the budget check entirely -- which is what
happened on every single submission before this. Verified end-to-end
against real Postgres: seeded a real budget line with an issued PO
(300,000), a draft PO (200,000, correctly excluded), and a cancelled
PO (500,000, correctly excluded) against it, confirmed the computed
remaining figure (700,000) is exactly right, then submitted a real
Purchase Request for 800,000 with *no* `remaining_budget` supplied at
all and watched it get correctly blocked -- proving the number was
computed, not passed in -- and confirmed a 100,000 request against the
same real remaining budget correctly succeeds.

11 new tests, plus the existing workflow and notifications suites and
the full 76-test tenant-isolation suite all re-run afterward
confirming nothing regressed from the PRC service signature change
(`remaining_budget` is now optional, not required -- backward
compatible; a caller that already supplies its own value is still
honored as-is, confirmed by a dedicated test).

## Extending the Workflow Engine to a second module -- CTM Contract Amendments

The Workflow Engine had exactly one real integration (PRC Purchase
Requests) since it was built. Extending it to a second module, chosen
deliberately: CTM's `ContractAmendment` had `approved_by`/
`approved_at` fields that looked like a real approval control, but
reading the actual code (`record_amendment`) found it wasn't one --
every amendment self-approved immediately on creation, with
`approved_by` always set to the same actor who created it. Any single
user holding `ctm:approve` could change a contract's value or
completion date alone, no second approval required, ever. This was a
real, unprotected control gap found by extending the engine, not a
gap invented to justify the work.

Fixed the same way PRC was integrated: purely additive, backward
compatible. A tenant that hasn't configured a workflow for
`("ctm", "contract_amendment")` sees identical behavior to before --
`get_active_workflow` returns `None`, the amendment self-approves and
applies immediately, exactly as it always has. Once a tenant
activates one, `record_amendment` creates the amendment `pending`
instead and starts a real workflow instance; the actual contract
mutation (the `completion_date`/`contract_value` change) is deferred
to a new `finalize_amendment` function, which the same pattern as
PRC's `approve_purchase_request` gates on the workflow's real status --
blocked with a clear redirect while pending, applies the mutation only
once the engine reports approved.

Required one real, honest schema change: `ContractAmendment` had no
status concept at all before this (nothing to represent "pending" --
the model was built assuming approval was instantaneous). Migration
`0034` adds it, defaulting every existing row to `'approved'` to
preserve exact historical behavior for amendments already recorded
before this existed.

Verified end-to-end against real Postgres, both paths: with no
workflow configured, a price amendment still self-approves and the
contract value updates immediately, unchanged from before. With a
workflow activated, a new price amendment is created pending, the
contract value provably does NOT change until the workflow instance
is actually approved, finalizing before that approval is correctly
blocked with the same "use the real workflow endpoint" redirect PRC
uses, and finalizing after approval correctly applies the deferred
mutation -- confirmed for both amendment types that have a real
contract-field effect (price and time/EOT).

6 new tests, plus the workflow/notifications/commitments suites (31
tests) and the full 76-test tenant-isolation suite all re-run
afterward confirming nothing regressed. The migration surfaced a real,
worth-noting distinction during its own production-safety check: `ALTER
TABLE ADD COLUMN` requires table *ownership*, not just DDL privilege
on the schema -- this only failed locally because this sandbox's
tables were created via a superuser connection during setup, not the
same role later applying migrations. Production's single
Render-provisioned role creates every table AND runs every migration,
so it already owns everything it created; confirmed by matching local
ownership to that reality and re-running, which then succeeded cleanly
-- documented in the migration file itself for anyone hitting the same
apparent failure later.

## Extending the Workflow Engine to a third module -- HSE Permits to Work

Third real integration, found the same way as CTM's: reading the
actual code rather than assuming. This one is more serious than CTM's
-- it's about physical worker safety, not just financial approval.
Every Permit to Work -- including hot work, confined space entry, and
working at height -- self-approved immediately on creation, and
`approved_by` was never even set: the route never passed an actor to
the service at all. Any single user holding `hse:write` could
self-issue an "approved" permit for hazardous work with zero recorded
accountability for who approved it.

Two independent fixes, both effective immediately, regardless of
whether a workflow is configured:

1. `actor_id` is now required and recorded as `approved_by`, even in
   the no-workflow path -- pure accountability, no change to gating
   behavior. This alone closes the "who approved this" half of the
   gap for every tenant, today.
2. Same pattern as PRC and CTM: when a tenant configures and activates
   a workflow for `("hse", "permit_to_work")`, the permit is created
   `draft` instead of self-approving, and a new
   `finalize_permit_approval` function (mirroring
   `approve_purchase_request` and `finalize_amendment`) only marks it
   approved once the workflow reports done. Downstream, `activate_permit`
   already required `status == "approved"` -- so this one integration
   point automatically makes the *entire* rest of the permit lifecycle
   (activation, and therefore the hazardous work itself) correctly
   wait on real approval, without needing to touch `activate_permit`
   at all.

No migration needed this time -- unlike CTM, `PermitToWork` already
had a `"draft"` status defined in `PERMIT_STATUSES`; it simply had
never been used, since every permit always skipped straight to
`"approved"`.

Verified end-to-end against real Postgres, both paths, including the
downstream activation gate: with no workflow configured, a hot-work
permit still self-approves immediately, but `approved_by` is now
correctly populated (confirmed it was always `None` before this fix).
With a workflow activated, a confined-space permit is created `draft`,
attempting to activate it before approval is correctly blocked (409),
finalizing before the workflow approves is correctly blocked with the
same real-endpoint redirect PRC and CTM use, and -- after genuine
workflow approval -- finalizing succeeds and the permit can then
actually be activated.

5 new tests, plus the full 42-test workflow/notifications/commitments/
CTM suite and the full 76-test tenant-isolation suite both re-run
afterward confirming nothing regressed. Three real module integrations
now exist (PRC, CTM, HSE), all following the same documented pattern
in `docs/WORKFLOW_ENGINE.md`.

## Real PRC-to-Inventory integration -- closing a stale gap found in a production-hardening audit

Requested a full production-hardening pass (34 sections, genuinely
weeks of real work, several sections not executable from this
sandbox at all -- no Flutter SDK, no real S3/email/SMS/CI to verify
against). Did Phase 1 for real instead of assuming: searched the
entire backend for TODO/FIXME/NotImplementedError/stub/mock patterns.
Found a real, stale one: `confirm_goods_receipt`'s own comment said
"once Module 8 (Inventory & Warehouse) exists, call its service here"
-- but Module 8 has existed since early in this build. Confirming a
GRN never actually updated real Inventory stock; the comment was just
never revisited once the module it was waiting for was actually
built.

Closed it for real. Two new nullable columns (migration `0035`), both
loose references matching the established pattern on the same tables
(`boq_item_id`/`cbs_line_item_id`): `PurchaseOrderLine.material_item_id`
(which Inventory material, if any, this line represents -- nullable,
since not every PO line is a trackable material; services and
subcontractor labor have nothing to receive into stock) and
`GoodsReceiptNote.warehouse_id` (which warehouse received the goods).
`confirm_goods_receipt` now calls Inventory's own `receive_stock`
service directly for any line with a `material_item_id` set -- PRC
still doesn't touch `inv_*` tables directly (SRS 3.3 bounded-context
discipline preserved), it calls Inventory's own function, the same
pattern used everywhere else cross-module integration exists in this
codebase. If a GRN has inventory-tracked lines but no `warehouse_id`,
confirmation is blocked with a clear, specific error rather than
silently skipping the stock update.

Verified end-to-end against real Postgres: confirmed stock starts at
zero, confirmed a GRN with a material-tracked line and no
`warehouse_id` is correctly blocked (not silently ignored), confirmed
providing one lets confirmation succeed and real Inventory stock
correctly reflects the receipt (`quantity_on_hand` and
`average_unit_cost` both exactly right), confirmed receiving the same
material twice accumulates correctly rather than overwriting, and
confirmed a GRN for a non-material (service/labor) PO line still
confirms fine with no warehouse required at all -- exact pre-existing
behavior preserved for the case this integration doesn't apply to.

4 new tests, plus the full 76-test tenant-isolation suite re-run
afterward confirming nothing regressed. The migration's `ADD COLUMN`
step required matching local table ownership to what production
already has (the single Render role that creates and later alters
every table it owns) -- the same known, previously-documented sandbox-
only quirk from the CTM migration, not a new issue.

Also fixed in the same audit pass: a genuine `NotImplementedError` in
`bdc/services.py::win_loss_summary` (BDC-11 Win/Loss reporting),
previously unreachable from any route. See the commit immediately
before this one for that fix's own full detail.

## Real EQP Celery task -- closing the last documented gap from the production-hardening audit

Third and last fix from that same audit pass. `approve_transfer`'s own
...
forever, with nothing ever coming back to actually move the
equipment's cost allocation on the date everyone had agreed it should
move.

Added `cutover_applied_at` to `EquipmentTransfer` (migration `0036`)
-- tracks whether the real effect has happened yet, distinct from
`status` staying `"approved"` regardless. Factored the actual cutover
effect out into a shared `apply_transfer_cutover` function so both an
immediate same-day cutover (at approval time) and a later one (applied
by the new task) go through the exact same code path, not two
almost-identical copies. A new `eqp.apply_due_transfer_cutovers`
Celery task, registered in the daily beat schedule alongside INV's
existing reorder-check task, following that task's own established
pattern exactly (per-tenant loop, one tenant's failure doesn't abort
the rest, returns a summary dict).

Two real bugs found and fixed by this migration's own production-
safety check, not by inspection: a bare cross-tenant backfill UPDATE
hit the exact FORCE RLS wall documented at length in migration
`0030`'s own history (fixed the same way, looping per-tenant instead);
and, separately, the migration's revision-ID string was literally too
long for `alembic_version`'s `varchar(32)` column -- a new, previously
unseen failure mode in this project, caught immediately by actually
running the migration rather than assuming a descriptive name is
always safe.

Verified for real, twice: first through actual HTTP routes (approved
a transfer with a 5-day-future cutover, confirmed the equipment's
project genuinely doesn't move yet, then confirmed the task correctly
moves it once the date arrives), then by calling the real task
function directly twice in a row to prove genuine idempotency (first
call applies the cutover, second call correctly does nothing).

6 new tests, plus both Celery task suites and the full 76-test tenant-
isolation suite re-run afterward confirming nothing regressed. Writing
the permanent test file surfaced two more real bugs in the test code
itself along the way (not the app): a SQLAlchemy identity-map query
that silently dropped an explicit `tenant_id` filter after commits
inside a loop (worked around with raw SQL for the affected
assertions), and a classic `object.id` read before the object was ever
flushed, which is a Python-side default and stays `None` until then.

## Frontend type-safety audit -- one module fixed for real, the rest honestly scoped, not silently done

Checked a real number flagged earlier (`50` files using `any`/
`@ts-ignore`/`@ts-nocheck`) rather than leaving it as an unexamined
figure. First finding worth stating plainly: **zero** `@ts-ignore` or
`@ts-nocheck` exist anywhere in this frontend -- the entire number was
`: any` annotations, and every single instance sampled traced back to
the exact same root cause: `apiClient.get(...)`/`.post(...)` calls
with no generic type argument, so every response is
`AxiosResponse<any>` by default, and every consumer either inherits
that silently or annotates `: any` explicitly to be clear about it.
Not 49 independent problems -- one systemic, low-severity pattern
repeated 49 times.

Fixed BDC (the module already dug into) as a real, complete
demonstration of the fix, not a partial one: `types.ts` (new) mirrors
`backend/app/modules/bdc/schemas.py`'s real Client/Lead/Opportunity
shapes exactly, including the real enum values
(`LEAD_STATUSES`/`OPPORTUNITY_STAGES`) as TypeScript union types
rather than a bare `string`. Every `apiClient` call in `hooks.ts` now
carries an explicit generic type argument, so the real shape flows
through automatically to every consumer -- all 6 `: any` annotations
across `ClientsPage.tsx`/`LeadsPage.tsx`/`OpportunitiesPage.tsx`
removed, not worked around.

Verified for real, not assumed: `npx tsc -b` (the actual production
build's own type-check step) compiles the whole frontend cleanly with
zero errors -- confirming the real interfaces genuinely match how the
JSX already used these fields, not just that they exist. Full
`npm run build` and `npm test` both clean afterward (11/11 tests,
including 3 new ones guarding this specific module against
regressing back to `any`).

Honest about scope: this is 1 of 25 modules. The other ~46 files
carrying the same pattern are real, tracked, low-severity debt --
functionally correct code (the actual runtime data is exactly what
the backend sends; TypeScript just wasn't verifying the shape at
compile time), not a security or correctness bug, and not claimed as
fixed here. Extending this to the rest is mechanical and follows the
exact pattern demonstrated on BDC: real `types.ts` per module mirroring
its backend schemas, generic type arguments on every `apiClient` call,
remove the now-unnecessary `any` annotations, confirm with `tsc -b`.

## Real email dispatch via Gmail SMTP -- closing the last documented notification gap

The Workflow Engine's notification integration always documented
`_dispatch_external` as "a real, documented no-op" pending provider
credentials -- the user chose email over SMS and Gmail SMTP as the
provider, so this closes that gap for real, not with another
placeholder.

`app/notifications/email.py` -- standard library only (`smtplib` +
`email.mime`), no new dependency. Configured via `SMTP_HOST` (defaults
to `smtp.gmail.com`), `SMTP_PORT`, `SMTP_USE_TLS`, `SMTP_USERNAME`,
`SMTP_PASSWORD`, `SMTP_FROM_ADDRESS` (falls back to `SMTP_USERNAME`,
since Gmail's SMTP server rejects a From address that isn't the
authenticated account or a verified alias). Empty by default: a
tenant/deployment that hasn't set `SMTP_USERNAME`/`SMTP_PASSWORD` yet
gets a clean no-op, not a crash -- in-app notifications alone keep
working regardless.

`app/notifications/tasks.py` -- a real Celery task
(`notifications.send_email`) with actual retry behavior (3 attempts,
30s apart) for transient SMTP failures. Takes plain values
(`to_address`, `subject`, `body`) rather than a notification ID to
re-query -- deliberately, to avoid a real race: `.delay()` can hand a
task to a worker before the calling request's own transaction has
committed, so a row-ID-based task could run before the row exists.
Passing everything the task needs directly sidesteps that; the task
never touches the database at all.

`_dispatch_external` now looks up the recipient's real email
(tenant-scoped, from the already-open transaction -- fast and safe to
do inline, unlike the network call itself) and queues the task rather
than sending synchronously, so a slow or down SMTP server can never
block the request that triggered the notification -- exactly the
requirement stated when this was still a no-op.

The Workflow Engine's own notification calls (approval-requested,
approved, rejected) now request `channel="email"` in addition to the
in-app record they already created, since a workflow approval is
exactly the kind of thing worth reaching someone by email, not just an
in-app bell they might not be watching.

Verified for real, not by inspection: ran an actual local SMTP test
server (`aiosmtpd`, AUTH-capable) and sent real messages through the
complete real chain three times -- `send_email()` directly (confirmed
the receiving server got the exact correct sender, recipient, subject,
and base64-decoded body), the real Celery task directly, and the full
`notify(channel="email")` → recipient lookup → task → SMTP path
end-to-end, including a real seeded tenant and user. All three
matched exactly. The permanent test suite (6 new tests) uses mocking
rather than a live server -- appropriate once the real integration is
already proven, not a substitute for having proven it.

Full regression: all 31 existing workflow/notification/CTM/HSE
integration tests and the full 76-test tenant-isolation suite both
re-run afterward confirming nothing regressed from adding
`channel="email"` to the Workflow Engine's calls.

## Extending the Workflow Engine to a fourth module -- EST Budget Revisions (and an honest dead end in Billing first)

Went looking for the next candidate deliberately, not just picking the
first thing that compiled. Checked Billing first, as planned --
`ProgressCertificate`, `VariationOrder`, and `Claim` all have an
`approved_by`/`reviewed_by` field, but every one of them is typed as a
free-text string ("often an external client/consultant name"), not a
reference to an internal user. Read the actual service functions to
confirm: Billing's approvals are honestly, consistently modeling
*external* client/consultant sign-off, not an internal staff chain --
the Workflow Engine's approver types (`specific_user`/`specific_role`)
only resolve internal users in this tenant, so there's no faithful way
to represent "the external client approves" through it. Forcing an
integration there would have been architecturally wrong, not just
"not chosen yet" -- so it wasn't forced. Stated here plainly rather
than silently skipped.

Searched more broadly instead for genuinely internal, UUID-referenced
`approved_by` fields across every module, and found a real, higher-
value one: `est/models.py:BudgetRevision`, explicitly documented as
"the only sanctioned way to change an approved CBS baseline" --
directly connected to `app/commitments/services.py`'s remaining-budget
computation from earlier this session. Same exact self-approval bug as
CTM and HSE before it: every revision self-approved immediately,
mutating `CBSLineItem.budgeted_amount` on the say-so of a single user
holding `est:approve` alone, no real second check. Fixed the same way,
purely additively: a tenant with no workflow configured keeps exact
prior behavior; one with a chain configured for
`("est", "budget_revision")` gets the revision created pending, the
budget mutation deferred to a new `finalize_budget_revision`, gated on
real approval the same way `finalize_amendment` and
`finalize_permit_approval` already are.

Verified end-to-end against real Postgres, and this test incidentally
proved something else for free: checked the resulting budget figure
through `app/commitments/services.py`'s own API
(`GET /v1/commitments/cbs-line-items/<id>/summary`) rather than
querying the table directly, confirming that module correctly reflects
a real budget change made through a completely different code path,
not just the scenario it was originally tested against. Confirmed the
budget stays at 1,200,000 while a second revision to 1,500,000 sits
pending, confirmed finalizing before approval is correctly blocked,
and confirmed it correctly becomes 1,500,000 only after genuine
workflow approval and finalization.

5 new tests, plus the full 76-test tenant-isolation suite re-run
afterward confirming nothing regressed. Migration revision ID kept
short deliberately (`0037_est_revision_status`, 24 characters) --
learned the hard way from `0036`'s own history that
`alembic_version.version_num` is `varchar(32)`.

Four real module integrations now exist (PRC, CTM, HSE, EST), all
following the same documented pattern in `docs/WORKFLOW_ENGINE.md`.

## Frontend type-safety audit, second module -- PRC, and a real bug the types caught

Extending the pattern from BDC to PRC -- the larger, more complex of
the two: 4 page components, 7 distinct backend entities (Vendor,
PurchaseRequest, PurchaseOrder, PurchaseOrderLine, POApprovalStep,
InvoiceMatch, GoodsReceiptNote), and one endpoint
(`GET /prc/purchase-orders/<id>`) whose real response shape is hand-
assembled by the route rather than a single schema dump -- checked
`app/modules/prc/routes.py` directly to get `line_items`/
`latest_match`/`approval_steps` right rather than guessing from the
schema alone.

Adding real types caught a genuine bug immediately, not a false
alarm: `po.latest_match && (...)` correctly narrows
`po.latest_match` from `InvoiceMatch | null` within that JSX block,
but a nested `onClick` closure inside it doesn't inherit that
narrowing -- a well-known, real TypeScript control-flow limitation
for closures, not a flaw in the new type definitions. Previously
invisible under `any`, since `any` has no null to narrow away in the
first place. Fixed by capturing the id into a local `const` before
the closure rather than suppressing the error.

All 9 `: any` annotations across the 4 page components removed, not
worked around. `npx tsc -b` compiles cleanly, full `npm run build`
and `npm test` both clean afterward (14/14 tests, 3 new ones guarding
this module against regressing back to `any`, following the same
pattern as BDC's).

2 of 25 modules now have real types. The rest remain honestly tracked
debt, not silently claimed as done -- same mechanical pattern to
extend: real `types.ts` mirroring the backend schemas (checking the
actual route code when a response is hand-assembled, not just the
schema), generic type arguments on every `apiClient` call, remove the
resulting unnecessary `any` annotations, confirm with `tsc -b`.

## react-router v6 -> v7, deferred repeatedly, finally done

The one security-relevant gap kept deliberately deferred every time
it came up earlier in this project's history -- real, breaking work
across every route in every module was the assumption. Checked current,
real documentation before touching anything rather than trusting that
assumption: React Router v7 has no breaking API changes for apps that
don't use the v7 data-router APIs (`createBrowserRouter`, loaders,
actions) -- and this codebase confirmed it uses none of that, only
plain declarative routing (`BrowserRouter`/`Routes`/`Route`/
`Navigate`/`Link`/`NavLink`/`Outlet`/`useNavigate`/`useParams`) across
35 files. The real scope turned out to be a fraction of what the
"v6->v7 migration" framing implied.

Also confirmed something not obvious from the version number alone:
`react-router-dom` (the package this app already depends on) still
exists as a real, maintained package at v7.18.2, kept around
specifically as a compatibility layer re-exporting everything so v6
apps need zero import changes -- it's only fully dropped at v8, a
separate, newer major not part of this migration's scope (the actual
latest `react-router` package is already at v8, which was *not* the
documented goal here). Stayed on `react-router-dom@^7` deliberately,
not the bare `react-router` package, for exactly that reason: it
closes the real CVEs (an open-redirect bypass and an arbitrary
constructor-injection issue in the 6.0.0-7.17.0 range) with the
smallest possible footprint matching what was actually asked for.

Verified for real: `npm audit` before showed the react-router CVEs
explicitly; after the bump, both are completely gone from the report
(4 remaining vulnerabilities are unrelated dev-tooling dependencies --
`js-yaml`, `nanoid`, `esbuild` -- not touched in this pass). `npx tsc -b`
compiled cleanly with zero source changes required anywhere across
all 35 files. Full `npm run build` and `npm test` both clean
afterward (15/15 tests, including the two that actually render real
pages through real routing, not just compile-time checks, plus one
new test guarding the dependency version itself against a future
...
- The frontend `npm audit` previously reported 21 vulnerabilities
  ...
  to 18 vulnerabilities, 0 critical, all remaining moderate/high in
  `react-router-dom`. **Update: fixed in a later session** -- see the
  "react-router v6 -> v7" session note further down. The real scope
  turned out much smaller than "a v6→v7 migration across every route
  in every module" implied: this codebase used only plain declarative
  routing, no v7 data-router APIs, so `react-router-dom@^7` (which
  still exists specifically as a compatibility layer) resolved the
  CVEs with zero source changes required anywhere. (Note: Vitest was
  reintroduced deliberately in a later session as a real, used
  dependency with a real test suite behind it — see the test-framework
  session notes above — which is different from the earlier unused,
  audit-flagged
...
- The new Workflow Engine (Module 26) has no visual builder UI, no
  automatic timeout/escalation enforcement (fields are recorded,
  nothing acts on them), and no org-hierarchy-based approver types,
  since no organizational hierarchy exists anywhere in this platform.
  Full detail in `docs/WORKFLOW_ENGINE.md`. Notification integration
  now exists (see above), and real email dispatch via Gmail SMTP is
  wired up and verified (see "Real email dispatch via Gmail SMTP"
  session note) -- SMS remains unimplemented by choice, not oversight.
  Four real module integrations exist so far (PRC Purchase Requests,
  CTM Contract Amendments, HSE Permits to Work, EST Budget Revisions)
  -- extending this to other modules follows the same documented
  pattern but hasn't been done for the rest of them yet. Billing was
  checked and found to be a genuine architectural mismatch (its
  approvals are external client/consultant sign-off, not internal
  staff chains), not a gap left for later.
